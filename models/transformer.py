...
class TransformerDecoderTSQPosEobjAttOccShift(nn.Module):
    ...
    def forwardt(self, tgt, memory,
        ...
        if shift_pass:
            ...
            hs_t = hs
            attt_all = atts_all
            sub_boxes_t = sub_boxes
            obj_boxes_t= obj_boxes
            verb_class_t = verb_class
            obj_class_t = obj_class
        else:
            assert False
...
class HOITransformerHQM(nn.Module):
...
class HOITransformerTSGBS(nn.Module):
    ...
    def forward(self, src, mask, query_embed1,
                query_embed_gt=None, query_embed_e_gt=None,
                query_embed_hard=None, query_embed_e_hard=None,
                query_embed2_mask=None,
                pos_embed=None):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        ...
        query_embed1 = query_embed1.unsqueeze(1).repeat(1, bs, 1)
        tgt1 = torch.zeros_like(query_embed1)
        if query_embed_gt is not None:
            sub_boxes, obj_boxes, obj_class, verb_class, \
            sub_boxes_t, obj_boxes_t, obj_class_t, verb_class_t, \
            sub_boxes_h, obj_boxes_h, obj_class_h, verb_class_h, \
            att_s, att_t, att_h, hs, hs_t, hs_h = \
                self.decoder.forwardt2(tgt1, memory,
                                      memory_key_padding_mask=mask,
                                      tgt_key_padding_mask_t=query_embed2_mask,
                                      pos=pos_embed,
                                      query_pos=query_embed1,
                                      query_embed_q=query_embed_gt,
                                      query_embed_e=query_embed_e_gt,
                                      query_embed_q_h=query_embed_hard,
                                      query_embed_e_h=query_embed_e_hard)
...
class TransformerDecoderTSQPosEobjAtt(nn.Module):
    ...
    def forwards(self, tgt, memory,
        ...
        return torch.stack(sub_boxes_t), torch.stack(obj_boxes_t), torch.stack(obj_class_t), torch.stack(verb_class_t), \
              torch.stack(attt_all), torch.stack(hs_t)
    ...
    def forwardt2(self, tgt, memory,
        ...
        output_h = query_embed_e_h
        for i in range(self.begin_l, len(self.layers)):
            layer = self.layers[i]
            output_h, att_h = layer(output_h, memory, tgt_mask=tgt_mask_t,
                                    memory_mask=memory_mask,
                                    tgt_key_padding_mask=tgt_key_padding_mask_t,
                                    memory_key_padding_mask=memory_key_padding_mask,
                                    pos=pos, query_pos=query_embed_q_h)
            if self.return_intermediate:
                output_n = self.norm(output_h)
                atth_all.append(att_h)
                hs_h.append(output_n)
        ...
        return torch.stack(sub_boxes), torch.stack(obj_boxes), torch.stack(obj_class), torch.stack(verb_class), \
               ...
               torch.stack(hs), torch.stack(hs_t), torch.stack(hs_h)
...
class HOITransformerTSQPOSEOBJAttDD(nn.Module):
    ...
    def forward(self, src, mask, query_embed1,
                query_embed_q=None, query_embed_e=None, query_embed_q2=None, query_embed_e2=None,
                query_embed2_mask=None,
                pos_embed=None):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        ...
        if query_embed_q is not None:
            ...
            sub_boxes_t, obj_boxes_t, obj_class_t, verb_class_t, att_s, att_t, hs, hs_t = \
                self.decoder.forwardt(tgt1, memory,
                                      ...
                                      query_embed_q=query_embed_q,
                                      query_embed_e=query_embed_e)
            sub_boxes_t2, obj_boxes_t2, obj_class_t2, verb_class_t2, att_t2, hs_t2 = \
                self.decoder.forwards(tgt1, memory,
                                      memory_key_padding_mask=mask,
                                      tgt_key_padding_mask_t=query_embed2_mask,
                                      pos=pos_embed,
                                      query_pos=query_embed1,
                                      query_embed_q=query_embed_q2,
                                      query_embed_e=query_embed_e2)

            return sub_boxes.transpose(1, 2), obj_boxes.transpose(1, 2), \
                   obj_class.transpose(1, 2), verb_class.transpose(1, 2), \
                   sub_boxes_t.transpose(1, 2), obj_boxes_t.transpose(1, 2), \
                   obj_class_t.transpose(1, 2), verb_class_t.transpose(1, 2), \
                   sub_boxes_t2.transpose(1, 2), obj_boxes_t2.transpose(1, 2), \
                   obj_class_t2.transpose(1, 2), verb_class_t2.transpose(1, 2), \
                   att_s, att_t, att_t2, hs.transpose(1, 2), hs_t.transpose(1, 2), hs_t2.transpose(1, 2)
    ...
    def forwardt(self, tgt, memory,
        ...
        for i in range(self.begin_l, len(self.layers)):
            ...
            output_t, att_t, att_select2 = layer(output_t, memory, tgt_mask=tgt_mask_t,
                                       ...
                                       tgt_key_padding_mask=tgt_key_padding_mask_t,
                                       memory_key_padding_mask=memory_key_padding_mask,
                                       pos=pos, query_pos=query_embed_q, attn_select=att_select_shift_all[i], use_hard_mask=True)
...
class TransformerDecoderAMM(nn.Module):
    ...
    def forwardt(self, tgt, memory,
        ...
        for i in range(self.begin_l, len(self.layers)):
            ...
            output_t, att_t, att_select2 = layer(output_t, memory, tgt_mask=tgt_mask_t,
                                       ...
                                       tgt_key_padding_mask=tgt_key_padding_mask_t,
                                       memory_key_padding_mask=memory_key_padding_mask,
                                       pos=pos, query_pos=query_embed_q, attn_select=att_select_all[i], use_hard_mask=True)
            att_select_shift_all.append(att_select2)
            if self.return_intermediate:
                output_n = self.norm(output_t)
                attt_all.append(att_t)
...
class TransformerDecoderLayerAMM(nn.Module):
...
class HOITransformerAMM(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        ###################################################################################
        decoder_norm = nn.LayerNorm(d_model)
        decoder_layer = TransformerDecoderLayerAMM(d_model, nhead, dim_feedforward,
                                                                dropout, activation, normalize_before)
        self.decoder = TransformerDecoderAMM(decoder_layer, num_decoder_layers, decoder_norm,
                                                                 return_intermediate=return_intermediate_dec,
                                                                 begin_l=begin_l,
                                                                 num_obj_classes=num_obj_classes,
                                                                 num_verb_classes=num_verb_classes)
...
def build_hoi_transformer_HQM(args, begin_l, num_obj_classes, num_verb_classes):
    return HOITransformerHQM(
        d_model=args.hidden_dim,
        dropout=args.dropout,
        nhead=args.nheads,
        dim_feedforward=args.dim_feedforward,
...
def build_hoi_transformer_TS_GBS(args, begin_l, num_obj_classes, num_verb_classes):
    return HOITransformerTSGBS(
        d_model=args.hidden_dim,
        dropout=args.dropout,
        nhead=args.nheads,
        dim_feedforward=args.dim_feedforward,
...
def build_hoi_transformer_AMM(args, begin_l, num_obj_classes, num_verb_classes):
    return HOITransformerAMM(
        d_model=args.hidden_dim,
        dropout=args.dropout,
        nhead=args.nheads,
        dim_feedforward=args.dim_feedforward,
...
def build_hoi_transformer_ts_qpos_eobj_attention_map_d(args, begin_l, num_obj_classes, num_verb_classes):
    return HOITransformerTSQPOSEOBJAttDD(
        d_model=args.hidden_dim,
        dropout=args.dropout,
        nhead=args.nheads,
        dim_feedforward=args.dim_feedforward,
