...
from models.transformer import build_hoi_transformer_TS_GBS
...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0, flags=None):
        ...
        if gt_items is not None:
            ...
            for i in range(len(gt_items)):
                ...
                gt_sp_vec[0:len(item), i, :12] = item[:, 512:512 + 12]
                gt_q_mask[i, 0:len(item)] = False
            orig_target_sizes = torch.stack([t["orig_size"] for t in target], dim=0)
            hard_sp_vec = self.augmented_bbox(gt_sp_vec, orig_target_sizes, epoch)

            hard_sp = torch.tanh(self.query_embed_sp(hard_sp_vec))
            hard_obj_emb = torch.zeros_like(hard_sp)
        ...
        if  gt_items is not None:
            ...
            outputs_hard_sub_coord, outputs_hard_obj_coord, outputs_hard_obj_class, outputs_hard_verb_class, \
            att, att_gt, att_hard, hs, hs_gt, hs_hard = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                      query_embed_gt=t_sp, query_embed_e_gt=t_obj_emb,
                                                      query_embed_hard=hard_sp, query_embed_e_hard=hard_obj_emb,
            ...
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   ...
                   'h_obj_logits': outputs_hard_obj_class[-1], 'h_verb_logits': outputs_hard_verb_class[-1],
                   'h_sub_boxes': outputs_hard_sub_coord[-1], 'h_obj_boxes': outputs_hard_obj_coord[-1],
                   'hs': hs[-1], 'hs_gt': hs_gt[-1], 'hs_h': hs_hard[-1],
                   'att': att[-1], 'att_gt': att_gt[-1], 'att_h': att_hard[-1]
                   }
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                    ...
                                                    outputs_hard_obj_class, outputs_hard_verb_class,
                                                    outputs_hard_sub_coord, outputs_hard_obj_coord,
                                                    hs, hs_gt, hs_hard,  att, att_gt, att_hard)
        else:
            outputs_sub_coord, outputs_obj_coord, outputs_obj_class, outputs_verb_class, \
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
                      ...
                      outputs_hard_obj_class, outputs_hard_verb_class, outputs_hard_sub_coord, outputs_hard_obj_coord,
                      hs, hs_gt, hs_hard,  att, att_gt, att_hard):
        ...
        ans2 = [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                 ...
                 'h_obj_logits': a2, 'h_verb_logits': b2, 'h_sub_boxes': c2, 'h_obj_boxes': d2,
                 'hs': h1, 'hs_gt': h2, 'hs_h': h3, 'att': att1, 'att_gt': att2, 'att_h': att3}
                ...
                zip(outputs_obj_class[self.begin_l:-1], outputs_verb_class[self.begin_l:-1],
                    ...
                    outputs_hard_obj_class[:-1], outputs_hard_verb_class[:-1],
                    outputs_hard_sub_coord[:-1], outputs_hard_obj_coord[:-1],
                    hs[self.begin_l:-1], hs_gt[:-1], hs_hard[:-1],
                    att[self.begin_l:-1], att_gt[:-1],  att_hard[:-1]
                    )]
        ans1.extend(ans2)
        return ans1
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_h_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        src_logits = outputs['h_obj_logits']
        target_classes_o = torch.cat([t['obj_labels'] for t in targets])
        src_logits = src_logits[indices_gt]
        if len(target_classes_o) == 0:
            loss_obj_ce = src_logits.sum()
            ...
            loss_obj_ce = F.cross_entropy(src_logits, target_classes_o, self.empty_weight)
        losses = {'loss_hard_obj_ce': loss_obj_ce}
        return losses
    ...
    def loss_h_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        src_logits = outputs['h_verb_logits']
        target_classes_o = torch.cat([t['verb_labels'] for t in targets])
        src_logits = src_logits[indices_gt]
        ...
        losses = {'loss_hard_verb_ce': loss_verb_ce}
        return losses
    ...
    def loss_h_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):

        src_sub_boxes = outputs['h_sub_boxes'][indices_gt]
        src_obj_boxes = outputs['h_obj_boxes'][indices_gt]
        target_sub_boxes = torch.cat([t['sub_boxes'] for t in targets], dim=0)
        target_obj_boxes = torch.cat([t['obj_boxes'] for t in targets], dim=0)
        ...
        losses = {}
        if src_sub_boxes.shape[0] == 0:
            losses['loss_hard_sub_bbox'] = src_sub_boxes.sum()
            losses['loss_hard_obj_bbox'] = src_obj_boxes.sum()
            losses['loss_hard_sub_giou'] = src_sub_boxes.sum()
            losses['loss_hard_obj_giou'] = src_obj_boxes.sum()
        else:
            loss_sub_bbox = F.l1_loss(src_sub_boxes, target_sub_boxes, reduction='none')
            loss_obj_bbox = F.l1_loss(src_obj_boxes, target_obj_boxes, reduction='none')
            losses['loss_hard_sub_bbox'] = loss_sub_bbox.sum() / num_interactions
            losses['loss_hard_obj_bbox'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() / (
                    exist_obj_boxes.sum() + 1e-4)
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                               box_cxcywh_to_xyxy(target_sub_boxes)))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                               box_cxcywh_to_xyxy(target_obj_boxes)))
            losses['loss_hard_sub_giou'] = loss_sub_giou.sum() / num_interactions
            losses['loss_hard_obj_giou'] = (loss_obj_giou * exist_obj_boxes).sum() / (exist_obj_boxes.sum() + 1e-4)
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, indices_gt, num, **kwargs):
        loss_map = {
            ...
            'hard_obj_labels': self.loss_h_obj_labels,
            'hard_verb_labels': self.loss_h_verb_labels,
            'hard_sub_obj_boxes': self.loss_h_sub_obj_boxes,
        }
...
def build(args):
    ...
    transformer = build_hoi_transformer_TS_GBS(args, begin_l=begin_l,
    ...
    weight_dict['loss_hard_obj_ce'] = args.obj_loss_coef
    weight_dict['loss_hard_verb_ce'] = args.verb_loss_coef
    weight_dict['loss_hard_sub_bbox'] = args.bbox_loss_coef
    weight_dict['loss_hard_obj_bbox'] = args.bbox_loss_coef
    weight_dict['loss_hard_sub_giou'] = args.giou_loss_coef
    weight_dict['loss_hard_obj_giou'] = args.giou_loss_coef
    ...
    losses = ['obj_labels', 'verb_labels', 'sub_obj_boxes', 'obj_cardinality',
              'gt_obj_labels', 'gt_verb_labels', 'gt_sub_obj_boxes', 'hard_obj_labels', 'hard_verb_labels', 'hard_sub_obj_boxes', 'match']
