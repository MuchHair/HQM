...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0):
        ...
        if self.aux_loss and gt_items is not None:
            ...
            gt_sp_vec_pos = self.augmented_bbox_pos(gt_sp_vec, orig_target_sizes, epoch)
            gt_sp_vec_neg = self.augmented_bbox_neg(gt_sp_vec, orig_target_sizes, epoch)
            gt_sp_vec = torch.cat((gt_sp_vec_pos, gt_sp_vec_neg), 0)
            neg_pos_ratio = 1
            gt_q_mask = gt_q_mask.repeat(1, neg_pos_ratio+1)
    ...
    def augmented_bbox_pos(self, ho_boxes_sp, shape, epoch=0):
    ...
    def augmented_bbox_neg(self, ho_boxes_sp, shape, epoch=0):
        ...
        if epoch <= 20:
            ...
            shift_r = 0.8
        neg_pos_ratio = 1
        ho_boxes_sp = ho_boxes_sp.repeat(neg_pos_ratio, 1, 1)
        ho_boxes = ho_boxes_sp[..., :8]
        ho_boxes = ho_boxes.transpose(1, 0)
        sub_boxes, obj_boxes = ho_boxes[..., :4], ho_boxes[..., 4:]
        img_h, img_w = shape.unbind(1)
        ...
        nzero_ind = (obj_boxes != 0).any(dim=-1) & (sub_boxes != 0).any(dim=-1)
        sub_boxes_out = torch.zeros_like(sub_boxes)
        obj_boxes_out = torch.zeros_like(obj_boxes)
        if len(nzero_ind):
            sub_boxes_nz = sub_boxes[nzero_ind]
            obj_boxes_nz = obj_boxes[nzero_ind]
            scale_fct_nz = scale_fct[nzero_ind]
            ...
            while not augmented1 and count1 < count_max:
                ...
                count1 += 1
            # sub_boxes[nzero_ind] = sub_boxes_nz
            sub_boxes_out[nzero_ind] = sub_boxes_nz
            ...
            # obj_boxes[nzero_ind] = obj_boxes_nz
            obj_boxes_out[nzero_ind] = obj_boxes_nz
            # print(count1, count2)
        sub_boxes = sub_boxes_out / scale_fct
        sub_boxes = box_xyxy_to_cxcywh(sub_boxes)
        obj_boxes = obj_boxes_out / scale_fct
        obj_boxes = box_xyxy_to_cxcywh(obj_boxes)
        c_dis = sub_boxes[..., 0:2] - obj_boxes[..., 0:2]
        a1 = sub_boxes[..., 2] * sub_boxes[..., 3]
        a2 = obj_boxes[..., 2] * obj_boxes[..., 3]
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx, log=True):
    ...
    def loss_gt_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx, log=True):
        src_logits = outputs['gt_obj_logits']
        #target_classes_o = torch.cat([t['obj_labels'] for t in targets])
        # src_logits = src_logits[indices_gt]

        target_classes_p = torch.cat([t['obj_labels'] for t in targets])
        src_logits_p = src_logits[indices_gt]

        src_logits_n = src_logits[neg_idx]
        target_classes_n = torch.full(src_logits_n.shape[:1], self.num_obj_classes,
                                    dtype=torch.int64, device=src_logits.device)

        src_logits = torch.cat((src_logits_p, src_logits_n), 0)
        target_classes_o = torch.cat((target_classes_p, target_classes_n), 0)
    ...
    @torch.no_grad()
    def loss_obj_cardinality(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx):
    ...
    def loss_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx):
    ...
    def loss_gt_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx):
        src_logits = outputs['gt_verb_logits']
        target_classes_p = torch.cat([t['verb_labels'] for t in targets])
        src_logits_p = src_logits[indices_gt]

        src_logits_n = src_logits[neg_idx]
        target_classes_n = torch.zeros_like(src_logits_n)
    ...
    def loss_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx):
    ...
    def loss_gt_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions, neg_idx):
    ...
    def get_loss(self, loss, outputs, targets, indices, indices_gt, num, neg_idx, **kwargs):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, indices_gt, num, neg_idx, **kwargs)
    ...
    def forward(self, outputs, targets):
        ...
        for i, t in enumerate(targets):
            ...
            batch_idx_neg.extend([i] * len(t['sub_boxes'])*neg_pos_ratio)
            for j in range(1, neg_pos_ratio+1):
                batch_idx1_neg.extend(torch.arange(j*len(t['sub_boxes']), (j+1)*len(t['sub_boxes'])))
        assert len(batch_idx1_neg) == len(batch_idx_neg)
        gt_idx = []
        gt_idx.append(batch_idx)
        gt_idx.append(batch_idx1)

        neg_idx = []
        neg_idx.append(batch_idx_neg)
        neg_idx.append(batch_idx1_neg)
        # Compute all the requested losses
        losses = {}
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, targets, indices, gt_idx, num_interactions, neg_idx))
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == 'obj_labels':
                        kwargs = {'log': False}
                    l_dict = self.get_loss(loss, outputs, targets, indices, gt_idx, num_interactions, neg_idx, **kwargs)
