...
from models.transformer import build_hoi_transformer_ts_qpos_eobj_attention_map_hardm_attn_each_p
...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0):
        ...
        ##################################################################################################
        t_sp = self.query_embed.weight.unsqueeze(1).repeat(1, src.shape[0], 1)
        t_obj_emb = torch.zeros_like(t_sp)
        if self.aux_loss:
            outputs_sub_coord, outputs_obj_coord, outputs_obj_class, outputs_verb_class, \
            outputs_hq_sub_coord, outputs_hq_obj_coord, outputs_hq_obj_class, outputs_hq_verb_class, \
            att, att_hq, hs, hs_hq, att_mask = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                                query_embed_q=t_sp, query_embed_e=t_obj_emb,
                                                                query_embed2_mask=None,
                                                                pos_embed=pos[-1])
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1],
                   'hq_obj_logits': outputs_hq_obj_class[-1], 'hq_verb_logits': outputs_hq_verb_class[-1],
                   'hq_sub_boxes': outputs_hq_sub_coord[-1], 'hq_obj_boxes': outputs_hq_obj_coord[-1],
                   'hs': hs[-1], 'hs_hq': hs_hq[-1],
                   'att': att[-1], 'att_hq': att_hq[-1],
                   'mask_rate': att_mask.mean(0).mean(-1).mean(-1) / (~mask).flatten(1).float().mean(-1)  # (d,bs,q,hw)->bs
                   }
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                    outputs_sub_coord, outputs_obj_coord,
                                                    outputs_hq_obj_class, outputs_hq_verb_class,
                                                    outputs_hq_sub_coord, outputs_hq_obj_coord,
                                                    hs, hs_hq, att, att_hq)
        else:
            outputs_sub_coord, outputs_obj_coord, outputs_obj_class, outputs_verb_class, \
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
                      outputs_hq_obj_class, outputs_hq_verb_class, outputs_hq_sub_coord, outputs_hq_obj_coord,
                      hs, hs_hq, att, att_hq):
        ...
        ans2 = [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                 'hq_obj_logits': a1, 'hq_verb_logits': b1, 'hq_sub_boxes': c1, 'hq_obj_boxes': d1,
                 'hs': h1, 'hs_hq': h2,  'att': att1, 'att_hq': att2}
                ...
                zip(outputs_obj_class[self.begin_l:-1], outputs_verb_class[self.begin_l:-1],
                    ...
                    outputs_hq_obj_class[:-1], outputs_hq_verb_class[:-1],
                    outputs_hq_sub_coord[:-1], outputs_hq_obj_coord[:-1],
                    hs[self.begin_l:-1], hs_hq[:-1],
                    att[self.begin_l:-1], att_hq[:-1]
                    )]
        ans1.extend(ans2)
        return ans1
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_hq_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        assert 'hq_obj_logits' in outputs
        src_logits = outputs['hq_obj_logits']
        ...
        losses = {'loss_hq_obj_ce': loss_obj_ce}
    ...
    def loss_hq_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        assert 'hq_verb_logits' in outputs
        src_logits = outputs['hq_verb_logits']
        ...
        losses = {'loss_hq_verb_ce': loss_verb_ce}
        return losses
    ...
    def loss_hq_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        assert 'hq_sub_boxes' in outputs and 'hq_obj_boxes' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_sub_boxes = outputs['hq_sub_boxes'][idx]
        src_obj_boxes = outputs['hq_obj_boxes'][idx]
        target_sub_boxes = torch.cat([t['sub_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        target_obj_boxes = torch.cat([t['obj_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        ...
        losses = {}
        if src_sub_boxes.shape[0] == 0:
            losses['loss_hq_sub_bbox'] = src_sub_boxes.sum()
            losses['loss_hq_obj_bbox'] = src_obj_boxes.sum()
            losses['loss_hq_sub_giou'] = src_sub_boxes.sum()
            losses['loss_hq_obj_giou'] = src_obj_boxes.sum()
        else:
            loss_sub_bbox = F.l1_loss(src_sub_boxes, target_sub_boxes, reduction='none')
            loss_obj_bbox = F.l1_loss(src_obj_boxes, target_obj_boxes, reduction='none')
            losses['loss_hq_sub_bbox'] = loss_sub_bbox.sum() / num_interactions
            losses['loss_hq_obj_bbox'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() / (
                    exist_obj_boxes.sum() + 1e-4)
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                               box_cxcywh_to_xyxy(target_sub_boxes)))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                               box_cxcywh_to_xyxy(target_obj_boxes)))
            losses['loss_hq_sub_giou'] = loss_sub_giou.sum() / num_interactions
            losses['loss_hq_obj_giou'] = (loss_obj_giou * exist_obj_boxes).sum() / (exist_obj_boxes.sum() + 1e-4)
        return losses
    ...
    @torch.no_grad()
    def loss_mask_rate(self, outputs, targets, indices, indices_gt, num_interactions):
        mask_rate = outputs['mask_rate'].mean()
        losses = {'mask_rate': mask_rate}
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, indices_gt, num, **kwargs):
        loss_map = {
            ...
            'hq_obj_labels': self.loss_hq_obj_labels,
            'hq_verb_labels': self.loss_hq_verb_labels,
            'hq_sub_obj_boxes': self.loss_hq_sub_obj_boxes,

            'match': self.loss_match_cosine,
            'match_att': self.loss_match_kl,

            'mask_rate': self.loss_mask_rate,
        }
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == "hq_obj_labels" and 'hq_obj_logits' not in aux_outputs: continue
                    if loss == "hq_verb_labels" and 'hq_verb_logits' not in aux_outputs: continue
                    if loss == "hq_sub_obj_boxes" and 'hq_sub_boxes' not in aux_outputs: continue
...
class PostProcessHOI(nn.Module):
    ...
    def forward(self, outputs, target_sizes, ids):
        ...
        if self.eval_gt:
            ...
            out_obj_logits, out_verb_logits, out_sub_boxes, out_obj_boxes = outputs['hq_obj_logits'], \
                                                                            outputs['hq_verb_logits'], \
                                                                            outputs['hq_sub_boxes'], \
                                                                            outputs['hq_obj_boxes']
            ...
            results_hq = []
            mask_rate = outputs['mask_rate']
            for os, ol, vs, sb, ob, id, mr in zip(obj_scores, obj_labels, verb_scores, sub_boxes, obj_boxes, ids,
                                                  mask_rate):
                sl = torch.full_like(ol, self.subject_category_id)
                l = torch.cat((sl, ol))
                b = torch.cat((sb, ob))
                results_hq.append({'labels': l.to('cpu').numpy(), 'boxes': b.to('cpu').numpy(), "image_id": id})
                vs = vs * os.unsqueeze(1)
                ids = torch.arange(b.shape[0])
                results_hq[-1].update({'verb_scores': vs.to('cpu').numpy(),
                                       'sub_ids': ids[:ids.shape[0] // 2].numpy(),
                                       'obj_ids': ids[ids.shape[0] // 2:].numpy(),
                                       'mask_rate': mr.to('cpu').numpy()})

            return results, results_hq
        return results
...
def build(args):
    ...
    transformer = build_hoi_transformer_ts_qpos_eobj_attention_map_hardm_attn_each_p(args, begin_l=begin_l,
    ...
    weight_dict['loss_hq_obj_ce'] = args.obj_loss_coef
    weight_dict['loss_hq_verb_ce'] = args.verb_loss_coef
    weight_dict['loss_hq_sub_bbox'] = args.bbox_loss_coef
    weight_dict['loss_hq_obj_bbox'] = args.bbox_loss_coef
    weight_dict['loss_hq_sub_giou'] = args.giou_loss_coef
    weight_dict['loss_hq_obj_giou'] = args.giou_loss_coef
    ...
    losses = ['obj_labels', 'verb_labels', 'sub_obj_boxes', 'obj_cardinality',
              'hq_obj_labels', 'hq_verb_labels', 'hq_sub_obj_boxes', 'mask_rate']
