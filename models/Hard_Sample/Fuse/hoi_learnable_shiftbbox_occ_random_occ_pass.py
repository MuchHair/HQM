...
from models.transformer import build_hoi_transformer_learnable_shiftbbox_occ_occ_pass
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0):
        ...
        if self.aux_loss and gt_items is not None:
            ...
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   ...
                   'shift_pass': shift_flag, 'occ_pass':occ_flag
                   }
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        ...
        losses = {'loss_obj_ce': loss_obj_ce*p}
    ...
    def loss_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        losses = {'loss_verb_ce': loss_verb_ce*p}
        return losses
    ...
    def loss_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        if src_sub_boxes.shape[0] == 0:
            ...
            losses['loss_sub_bbox'] = loss_sub_bbox.sum() * p / num_interactions
            losses['loss_obj_bbox'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() * p / (
                    exist_obj_boxes.sum() + 1e-4)
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                               box_cxcywh_to_xyxy(target_sub_boxes)))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                               box_cxcywh_to_xyxy(target_obj_boxes)))
            losses['loss_sub_giou'] = loss_sub_giou.sum() * p / num_interactions
            losses['loss_obj_giou'] = (loss_obj_giou * exist_obj_boxes).sum() * p / (exist_obj_boxes.sum() + 1e-4)
        return losses
...
def build(args):
    ...
    transformer = build_hoi_transformer_learnable_shiftbbox_occ_occ_pass(args, begin_l=begin_l,
    ...
    weight_dict['loss_occ_obj_ce'] = args.obj_loss_coef
    weight_dict['loss_occ_verb_ce'] = args.verb_loss_coef
    weight_dict['loss_occ_sub_bbox'] = args.bbox_loss_coef
    weight_dict['loss_occ_obj_bbox'] = args.bbox_loss_coef
    weight_dict['loss_occ_sub_giou'] = args.giou_loss_coef
    weight_dict['loss_occ_obj_giou'] = args.giou_loss_coef
