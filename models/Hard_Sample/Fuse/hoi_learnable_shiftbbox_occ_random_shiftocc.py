...
from models.transformer import build_hoi_transformer_learnable_shiftbbox_occ_shiftocc
from util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
from util.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       accuracy, get_world_size, is_dist_avail_and_initialized)
from torchvision.ops.boxes import box_area

streams = ['learnable_occ', 'shift_occ', 'shift']
shift_occ_flags = [True, False]
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0):
        ...
        occ_flag = False
        shift_occ = False
        # random.shuffle(shift_occ_flags)
        if stream=='learnable_occ':
            occ_flag = True
        elif stream=='shift':
            shift_flag = True
        elif stream=='shift_occ':
            shift_flag = True
            shift_occ = True
        ...
        if self.aux_loss and gt_items is not None:
            ...
            att, att_gt,  att_occ, hs, hs_gt, hs_occ = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                      ...
                                                      pos_embed=pos[-1], occ_embed_q=occ_embed_q, occ_embed_e=occ_embed_e, shift_occ_pass=shift_occ)
...
def build(args):
    ...
    transformer = build_hoi_transformer_learnable_shiftbbox_occ_shiftocc(args, begin_l=begin_l,
