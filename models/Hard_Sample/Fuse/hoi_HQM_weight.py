...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0, flags=[False, False]):
        gt_q_mask = None
        shift_flag = flags[0]
        occ_flag = flags[1]
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_occ_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        if outputs['occ_pass']:
            ...
            losses = {'loss_occ_obj_ce': loss_obj_ce}
        else:
            assert False
        ...
        if outputs['occ_pass']:
            ...
            if src_logits.shape[0] == 0:
                ...
                if self.verb_loss_type == 'bce':
                    ...
                    src_logits = src_logits.sigmoid()
                    loss_verb_ce = self._neg_loss(src_logits, target_classes)
            losses = {'loss_occ_verb_ce': loss_verb_ce}
    ...
    def loss_occ_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        if outputs['occ_pass']:
            ...
            if src_sub_boxes.shape[0] == 0:
                ...
                loss_sub_bbox = F.l1_loss(src_sub_boxes, target_sub_boxes, reduction='none')
                loss_obj_bbox = F.l1_loss(src_obj_boxes, target_obj_boxes, reduction='none')
                losses['loss_occ_sub_bbox'] = loss_sub_bbox.sum() / num_interactions
                losses['loss_occ_obj_bbox'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() / (
                        exist_obj_boxes.sum() + 1e-4)
                loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                                   box_cxcywh_to_xyxy(target_sub_boxes)))
                loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                                   box_cxcywh_to_xyxy(target_obj_boxes)))
                losses['loss_occ_sub_giou'] = loss_sub_giou.sum() / num_interactions
                losses['loss_occ_obj_giou'] = (loss_obj_giou * exist_obj_boxes).sum() / (exist_obj_boxes.sum() + 1e-4)
        else:
            assert False
        return losses
    ...
    def forward(self, outputs, targets):
        ...
        if outputs['shift_pass']:
            for loss in ['gt_obj_labels', 'gt_verb_labels', 'gt_sub_obj_boxes']:
                losses.update(self.get_loss(loss, outputs, targets, indices, gt_idx, num_interactions))
        if outputs['occ_pass']:
            for loss in ['occ_obj_labels', 'occ_verb_labels', 'occ_sub_obj_boxes']:
                losses.update(self.get_loss(loss, outputs, targets, indices, gt_idx, num_interactions))
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                if outputs['shift_pass']:
                    kk = 'gt'
                elif outputs['occ_pass']:
                    kk = 'occ'
                else:
                    assert False
                for loss in [f'{kk}_obj_labels', f'{kk}_verb_labels', f'{kk}_sub_obj_boxes']:
                    kwargs = {}
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, gt_idx, num_interactions, **kwargs)
                    l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                    losses.update(l_dict)
...
def build(args):
    ...
    transformer = build_hoi_transformer_HQM(args, begin_l=begin_l, num_obj_classes=args.num_obj_classes, num_verb_classes=args.num_verb_classes)
    ...
    w = 0.8
    weight_dict['loss_occ_obj_ce'] = args.obj_loss_coef * w
    weight_dict['loss_occ_verb_ce'] = args.verb_loss_coef * w
    weight_dict['loss_occ_sub_bbox'] = args.bbox_loss_coef * w
    weight_dict['loss_occ_obj_bbox'] = args.bbox_loss_coef * w
    weight_dict['loss_occ_sub_giou'] = args.giou_loss_coef * w
    weight_dict['loss_occ_obj_giou'] = args.giou_loss_coef * w

    weight_dict['loss_gt_obj_ce'] = args.obj_loss_coef * w
    weight_dict['loss_gt_verb_ce'] = args.verb_loss_coef * w
    weight_dict['loss_gt_sub_bbox'] = args.bbox_loss_coef * w
    weight_dict['loss_gt_obj_bbox'] = args.bbox_loss_coef * w
    weight_dict['loss_gt_sub_giou'] = args.giou_loss_coef * w
    weight_dict['loss_gt_obj_giou'] = args.giou_loss_coef * w
