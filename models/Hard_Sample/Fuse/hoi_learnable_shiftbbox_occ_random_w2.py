...
from models.transformer import build_hoi_transformer_learnable_shiftbbox_occ
...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0, occ_pass=False):
        gt_q_mask = None
        ...
        if gt_items is not None:
            ...
            gt_sp_vec = self.augmented_bbox(gt_sp_vec, orig_target_sizes, epoch)
        ...
        occ_embed_q = self.query_embed.weight.unsqueeze(1).repeat(1, src.shape[0], 1)
        occ_embed_e = torch.zeros_like(occ_embed_q)
        if gt_items is not None:
            ...
            outputs_occ_sub_coord, outputs_occ_obj_coord, outputs_occ_obj_class, outputs_occ_verb_class, \
            att, att_gt,  att_occ, hs, hs_gt, hs_occ = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                      query_embed_q=t_sp, query_embed_e=t_obj_emb,
                                                      query_embed2_mask=gt_q_mask,
                                                      pos_embed=pos[-1], occ_embed_q=occ_embed_q, occ_embed_e=occ_embed_e, shift_pass=shift_flag, occ_pass=occ_flag)
            ...
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   ...
                   'occ_obj_logits': outputs_occ_obj_class[-1], 'occ_verb_logits': outputs_occ_verb_class[-1],
                   'occ_sub_boxes': outputs_occ_sub_coord[-1], 'occ_obj_boxes': outputs_occ_obj_coord[-1],
                   'hs': hs[-1], 'hs_gt': hs_gt[-1], 'hs_occ': hs_occ[-1],
                   'att': att[-1], 'att_gt': att_gt[-1], 'att_occ': att_occ[-1],
                   'shift_pass': shift_flag, 'occ_pass':occ_flag, 'gt_sp_vec': gt_sp_vec
                   }
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                    ...
                                                    outputs_occ_obj_class, outputs_occ_verb_class,
                                                    outputs_occ_sub_coord, outputs_occ_obj_coord,
                                                    hs, hs_gt, hs_occ, att, att_gt, att_occ, shift_flag, occ_flag)
        else:
            outputs_sub_coord, outputs_obj_coord, outputs_obj_class, outputs_verb_class, \
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
                      ...
                      outputs_occ_obj_class, outputs_occ_verb_class, outputs_occ_sub_coord, outputs_occ_obj_coord,
                      hs, hs_gt, hs_occ, att, att_gt, att_occ, shift_flag, occ_flag):
        ...
        ans2 = [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                 ...
                 'occ_obj_logits': a2, 'occ_verb_logits': b2, 'occ_sub_boxes': c2, 'occ_obj_boxes': d2,
                 'hs': h1, 'hs_gt': h2,  'hs_occ': h3,  'att': att1, 'att_gt': att2, 'att_occ': att3, 'shift_pass': shift_flag, 'occ_pass': occ_flag}
                for a, b, c, d, a1, b1, c1, d1, a2, b2, c2, d2, h1, h2, h3, att1, att2, att3 in
                zip(outputs_obj_class[self.begin_l:-1], outputs_verb_class[self.begin_l:-1],
                    ...
                    outputs_occ_obj_class[:-1], outputs_occ_verb_class[:-1],
                    outputs_occ_sub_coord[:-1], outputs_occ_obj_coord[:-1],
                    hs[self.begin_l:-1], hs_gt[:-1], hs_occ[:-1],
                    att[self.begin_l:-1], att_gt[:-1], att_occ[:-1]
                    )]
        ans1.extend(ans2)
        return ans1
    ...
    def augmented_bbox(self, ho_boxes_sp, shape, epoch=0):
        ...
        if len(nzero_ind):
            ...
            while not augmented1 and count1 < count_max:
                ...
                if (iou_rate.float().sum() / len(aug_bbox)) >= shift_r or count1 == count_max:
                    augmented1 = True
            ...
            while not augmented2 and count2 < count_max:
                ...
                # print(f"iou_rate: {iou_rate}, count: {count}")
                if (iou_rate.float().sum() / len(aug_bbox_obj)) >= shift_r or count2 == count_max:
                    augmented2 = True
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_occ_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        if outputs['occ_pass']:
            p = 1
        else:
            p = 0
        src_logits = outputs['occ_obj_logits']
        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([t['obj_labels'][J] for t, (_, J) in zip(targets, indices)])
    ...
    def loss_gt_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        ...
        if len(target_classes_o) == 0:
            ...
            loss_obj_ce = F.cross_entropy(src_logits, target_classes_o, self.empty_weight)
        losses = {'loss_gt_obj_ce': loss_obj_ce*p}
        return losses
    ...
    def loss_occ_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        src_logits = outputs['occ_verb_logits']

        idx = self._get_src_permutation_idx(indices)
        target_classes = torch.cat([t['verb_labels'][J] for t, (_, J) in zip(targets, indices)])

        src_logits = src_logits[idx]
        if src_logits.shape[0] == 0:
            loss_verb_ce = src_logits.sum()
        else:
            if self.verb_loss_type == 'bce':
                loss_verb_ce = F.binary_cross_entropy_with_logits(src_logits, target_classes)
            elif self.verb_loss_type == 'focal':
        ...
        losses = {'loss_occ_verb_ce': loss_verb_ce*p}
        return losses
    ...
    def loss_gt_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        losses = {'loss_gt_verb_ce': loss_verb_ce*p}
        return losses
    ...
    def loss_occ_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        if outputs['occ_pass']:
            p = 1
        else:
            p = 0
        idx = self._get_src_permutation_idx(indices)
        src_sub_boxes = outputs['occ_sub_boxes'][idx]
        src_obj_boxes = outputs['occ_obj_boxes'][idx]
        target_sub_boxes = torch.cat([t['sub_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        target_obj_boxes = torch.cat([t['obj_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        ...
        losses = {}
        if src_sub_boxes.shape[0] == 0:
            losses['loss_occ_sub_bbox'] = src_sub_boxes.sum()
            losses['loss_occ_obj_bbox'] = src_obj_boxes.sum()
            losses['loss_occ_sub_giou'] = src_sub_boxes.sum()
            losses['loss_occ_obj_giou'] = src_obj_boxes.sum()
        else:
            loss_sub_bbox = F.l1_loss(src_sub_boxes, target_sub_boxes, reduction='none')
            loss_obj_bbox = F.l1_loss(src_obj_boxes, target_obj_boxes, reduction='none')
            losses['loss_occ_sub_bbox'] = loss_sub_bbox.sum() * p / num_interactions
            losses['loss_occ_obj_bbox'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() * p / (
                    exist_obj_boxes.sum() + 1e-4)
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                               box_cxcywh_to_xyxy(target_sub_boxes)))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                               box_cxcywh_to_xyxy(target_obj_boxes)))
            losses['loss_occ_sub_giou'] = loss_sub_giou.sum() * p / num_interactions
            losses['loss_occ_obj_giou'] = (loss_obj_giou * exist_obj_boxes).sum() * p / (exist_obj_boxes.sum() + 1e-4)
        return losses
    ...
    def loss_gt_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        if src_sub_boxes.shape[0] == 0:
            ...
            loss_sub_bbox = F.l1_loss(src_sub_boxes, target_sub_boxes, reduction='none')
            loss_obj_bbox = F.l1_loss(src_obj_boxes, target_obj_boxes, reduction='none')
            losses['loss_gt_sub_bbox'] = loss_sub_bbox.sum() * p/ num_interactions
            losses['loss_gt_obj_bbox'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() * p/ (
                    exist_obj_boxes.sum() + 1e-4)
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                               box_cxcywh_to_xyxy(target_sub_boxes)))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                               box_cxcywh_to_xyxy(target_obj_boxes)))
            losses['loss_gt_sub_giou'] = loss_sub_giou.sum() * p/ num_interactions
            losses['loss_gt_obj_giou'] = (loss_obj_giou * exist_obj_boxes).sum() * p / (exist_obj_boxes.sum() + 1e-4)
        return losses

    def loss_match_cosine(self, outputs, targets, indices, indices_gt, num_interactions):
        hs = outputs['hs'].detach()
        hs_gt = outputs['hs_gt']
    ...
    def get_loss(self, loss, outputs, targets, indices, indices_gt, num, **kwargs):
        loss_map = {
            ...
            'occ_obj_labels':self.loss_occ_obj_labels,
            'occ_sub_obj_boxes': self.loss_occ_sub_obj_boxes,
            'occ_verb_labels': self.loss_occ_verb_labels,
...
def build(args):
    ...
    transformer = build_hoi_transformer_learnable_shiftbbox_occ(args, begin_l=begin_l,
    ...
    weight_dict['loss_occ_obj_ce'] = args.obj_loss_coef * 2
    weight_dict['loss_occ_verb_ce'] = args.verb_loss_coef * 2
    weight_dict['loss_occ_sub_bbox'] = args.bbox_loss_coef * 2
    weight_dict['loss_occ_obj_bbox'] = args.bbox_loss_coef * 2
    weight_dict['loss_occ_sub_giou'] = args.giou_loss_coef * 2
    weight_dict['loss_occ_obj_giou'] = args.giou_loss_coef * 2

    weight_dict['loss_gt_obj_ce'] = args.obj_loss_coef * 2
    weight_dict['loss_gt_verb_ce'] = args.verb_loss_coef * 2
    weight_dict['loss_gt_sub_bbox'] = args.bbox_loss_coef * 2
    weight_dict['loss_gt_obj_bbox'] = args.bbox_loss_coef * 2
    weight_dict['loss_gt_sub_giou'] = args.giou_loss_coef * 2
    weight_dict['loss_gt_obj_giou'] = args.giou_loss_coef * 2
    ...
    losses = ['obj_labels', 'verb_labels', 'sub_obj_boxes', 'obj_cardinality',
              'gt_obj_labels', 'gt_verb_labels', 'gt_sub_obj_boxes', 'occ_obj_labels', 'occ_verb_labels', 'occ_sub_obj_boxes']
