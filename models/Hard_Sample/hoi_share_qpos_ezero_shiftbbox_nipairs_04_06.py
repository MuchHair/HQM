...
class SetCriterionHOI(nn.Module):
    ...
    def loss_gt_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        ...
        target_classes_o = torch.cat([torch.cat([t['obj_labels'], t['obj_labels_noni']]) for t in targets])
    ...
    def loss_gt_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        target_classes_o = torch.cat([torch.cat([t['verb_labels'], t['verb_labels_noni']]) for t in targets])
        src_logits = src_logits[indices_gt]
    ...
    def loss_gt_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        target_sub_boxes = torch.cat([torch.cat([t['sub_boxes'], t['sub_boxes_noni']]) for t in targets], dim=0)
        target_obj_boxes = torch.cat([torch.cat([t['obj_boxes'], t['obj_boxes_noni']]) for t in targets], dim=0)
    ...
    def forward(self, outputs, targets):
        ...
        for i, t in enumerate(targets):
            batch_idx.extend([i] * (len(t['sub_boxes']) + len(t['sub_boxes_noni'])))
            batch_idx1.extend(torch.arange(0, (len(t['sub_boxes']) + len(t['sub_boxes_noni']))))
        gt_idx = []
