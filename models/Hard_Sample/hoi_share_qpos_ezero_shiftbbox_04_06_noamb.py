    ...
    def augmented_bbox(self, ho_boxes_sp, shape, sub_ids, obj_ids, img_ids, epoch=0):
        ...
        if len(sub_boxes_nz):
            ...
            while not augmented1 and count1 < count_max:
                ...
                iou_aug_gt = self.box_iou(sub_boxes_nz, aug_bbox)
                iou_rate = (iou_aug_gt > thres_low) & (iou_aug_gt < thres_high)

                iou_nsame_id = (box_iou_mat(aug_bbox, all_boxes_nz)[0] * sub_nsame_mask).max(-1)[0]

                # print(f"iou_rate: {iou_rate}, count: {count}")
                if ((iou_rate.float().sum() / len(aug_bbox)) >= shift_r and
                        (iou_aug_gt > iou_nsame_id).all()) or count1 == count_max:
                    augmented1 = True
            ...
            while not augmented2 and count2 < count_max:
                ...
                iou_aug_gt = self.box_iou(obj_boxes_nz, aug_bbox_obj)
                iou_rate = (iou_aug_gt > thres_low) & (iou_aug_gt < thres_high)

                iou_nsame_id = (box_iou_mat(aug_bbox_obj, all_boxes_nz)[0] * obj_nsame_mask).max(-1)[0]
                # print(f"iou_rate: {iou_rate}, count: {count}")
                if ((iou_rate.float().sum() / len(aug_bbox_obj)) >= shift_r and
                        (iou_aug_gt > iou_nsame_id).all()) or count2 == count_max:
                    augmented2 = True
