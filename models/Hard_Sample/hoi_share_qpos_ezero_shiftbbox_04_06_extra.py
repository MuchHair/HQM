...
from models.transformer import build_hoi_transformer_ts_qpos_eobj_attention_map_extra
...
class DETRHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries,
                 aux_loss=False, begin_l=3, args=None):
        super().__init__()
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0):
        ...
        if self.aux_loss and gt_items is not None:
            ...
            t_sp_gt = torch.tanh(self.query_embed_sp(gt_sp_vec))
            gt_sp_vec_aug = self.augmented_bbox(gt_sp_vec, orig_target_sizes, epoch)
            t_sp = torch.tanh(self.query_embed_sp(gt_sp_vec_aug))
            t_sp_extra = self.pos_extrapolation(t_sp, t_sp_gt)
        ...
        if self.aux_loss and gt_items is not None:
            ...
            outputs_ep_sub_coord, outputs_ep_obj_coord, outputs_ep_obj_class, outputs_ep_verb_class, \
            att, att_gt, att_ep, hs, hs_gt, hs_ep = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                                     query_embed_q=t_sp, query_embed_e=t_obj_emb,
                                                                     query_embed_q_ep=t_sp_extra, query_embed_e_ep=t_obj_emb,
            ...
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   ...
                   'ep_obj_logits': outputs_ep_obj_class[-1], 'ep_verb_logits': outputs_ep_verb_class[-1],
                   'ep_sub_boxes': outputs_ep_sub_coord[-1], 'ep_obj_boxes': outputs_ep_obj_coord[-1],
                   'hs': hs[-1], 'hs_gt': hs_gt[-1], 'hs_ep': hs_ep[-1],
                   'att': att[-1], 'att_gt': att_gt[-1], 'att_ep': att_ep[-1]
                   }
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                    ...
                                                    outputs_ep_sub_coord, outputs_ep_obj_coord,
                                                    outputs_ep_obj_class, outputs_ep_verb_class,
                                                    hs, hs_gt, hs_ep, att, att_gt, att_ep)
        else:
            outputs_sub_coord, outputs_obj_coord, outputs_obj_class, outputs_verb_class, \
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
        ...
        ans2 = [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                 ...
                 'ep_obj_logits': a2, 'ep_verb_logits': b2, 'ep_sub_boxes': c2, 'ep_obj_boxes': d2,
                 'hs': h1, 'hs_gt': h2, 'hs_ep': h3,  'att': att1, 'att_gt': att2, 'att_ep': att3}
                ...
                zip(outputs_obj_class[self.begin_l:-1], outputs_verb_class[self.begin_l:-1],
                    ...
                    outputs_ep_obj_class[:-1], outputs_ep_verb_class[:-1],
                    outputs_ep_sub_coord[:-1], outputs_ep_obj_coord[:-1],
                    hs[self.begin_l:-1], hs_gt[:-1], hs_ep[:-1],
                    att[self.begin_l:-1], att_gt[:-1], att_ep[:-1]
                    )]
        ans1.extend(ans2)
        return ans1

    def pos_extrapolation(self, query_aug, query_gt, nzero_ind=None):
        simi_extra = torch.zeros(query_aug.shape[0]).bool()
        lambda_extra = 1.1
        while not simi_extra.all():
            query_extra = lambda_extra * query_aug + (1 - lambda_extra) * query_gt
            simi_extra = torch.cosine_similarity(query_extra, query_gt) <= torch.cosine_similarity(query_aug, query_gt)
            lambda_extra += 0.01
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_gt_obj_labels(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        ...
        src_logits_ep = outputs['ep_obj_logits']
        target_classes_o = torch.cat([t['obj_labels'] for t in targets])
        src_logits = src_logits[indices_gt]
        src_logits_ep = src_logits_ep[indices_gt]

        if len(target_classes_o) == 0:
            loss_obj_ce = src_logits.sum() + src_logits_ep.sum()
        else:
            src_logits = torch.cat([src_logits, src_logits_ep], dim=0)
            loss_obj_ce = F.cross_entropy(src_logits, target_classes_o.repeat(2), self.empty_weight)
    ...
    def loss_gt_verb_labels(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        src_logits_ep = outputs['ep_verb_logits']
        target_classes_o = torch.cat([t['verb_labels'] for t in targets]).repeat(2, 1)
        src_logits = src_logits[indices_gt]
        src_logits_ep = src_logits_ep[indices_gt]
    ...
    def loss_gt_sub_obj_boxes(self, outputs, targets, indices, indices_gt, num_interactions):
        ...
        src_sub_boxes_ep = outputs['ep_sub_boxes'][indices_gt]
        src_obj_boxes_ep = outputs['ep_obj_boxes'][indices_gt]
        target_sub_boxes = torch.cat([t['sub_boxes'] for t in targets], dim=0).repeat(2, 1)
        target_obj_boxes = torch.cat([t['obj_boxes'] for t in targets], dim=0).repeat(2, 1)

        exist_obj_boxes = (target_obj_boxes != 0).any(dim=1)

        src_sub_boxes = torch.cat([src_sub_boxes, src_sub_boxes_ep], dim=0)
        src_obj_boxes = torch.cat([src_obj_boxes, src_obj_boxes_ep], dim=0)
        losses = {}
        if src_sub_boxes.shape[0] == 0:
