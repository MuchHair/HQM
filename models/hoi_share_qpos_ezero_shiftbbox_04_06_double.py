...
from models.transformer import build_hoi_transformer_ts_qpos_eobj_attention_map_d
...
class DETRHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, gt_items=None, target=None, epoch=0, occ_pass=False):
        ...
        if gt_items is not None:
            ...
            gt_sp_vec1 = self.augmented_bbox(gt_sp_vec, orig_target_sizes, epoch)
            gt_sp_vec2 = self.augmented_bbox(gt_sp_vec, orig_target_sizes, epoch)
            t_sp = torch.tanh(self.query_embed_sp(gt_sp_vec1))
            t_sp2 = torch.tanh(self.query_embed_sp(gt_sp_vec2))
            # t_obj_emb = self.query_embed_obj(gt_obj_vec)
            t_obj_emb = torch.zeros_like(t_sp)
            t_obj_emb2 = torch.zeros_like(t_sp2)
        ##########################################################
        ...
        if gt_items is not None:
            ...
            outputs_gt_sub_coord2, outputs_gt_obj_coord2, outputs_gt_obj_class2, outputs_gt_verb_class2, \
            att, att_gt, att_gt2, hs, hs_gt, hs_gt2 = self.transformer(self.input_proj(src), mask, self.query_embed.weight,
                                                      query_embed_q=t_sp, query_embed_e=t_obj_emb, query_embed_q2=t_sp2, query_embed_e2=t_obj_emb2,
            ...
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                   ...
                   'gt_obj_logits2': outputs_gt_obj_class2[-1], 'gt_verb_logits2': outputs_gt_verb_class2[-1],
                   'gt_sub_boxes2': outputs_gt_sub_coord2[-1], 'gt_obj_boxes2': outputs_gt_obj_coord2[-1],
                   'hs': hs[-1], 'hs_gt': hs_gt[-1], 'hs_gt2': hs_gt2[-1],
                   'att': att[-1], 'att_gt': att_gt[-1], 'att_gt2': att_gt2[-1], 'gt_sp_vec': gt_sp_vec,
                   }
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                    ...
                                                    outputs_gt_obj_class2, outputs_gt_verb_class2,
                                                    outputs_gt_sub_coord2, outputs_gt_obj_coord2,
                                                    hs, hs_gt, hs_gt2, att, att_gt, att_gt2)
        else:
            outputs_sub_coord, outputs_obj_coord, outputs_obj_class, outputs_verb_class, \
    ...
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord,
                      ...
                      outputs_gt_obj_class2, outputs_gt_verb_class2, outputs_gt_sub_coord2, outputs_gt_obj_coord2,
                      hs, hs_gt, hs_gt2, att, att_gt, att_gt2):
        ...
        ans2 = [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d,
                 ...
                 'gt_obj_logits2': a2, 'gt_verb_logits2': b2, 'gt_sub_boxes2': c2, 'gt_obj_boxes2': d2,
                ...
                zip(outputs_obj_class[self.begin_l:-1], outputs_verb_class[self.begin_l:-1],
                    ...
                    outputs_gt_obj_class2[:-1], outputs_gt_verb_class2[:-1],
                    outputs_gt_sub_coord2[:-1], outputs_gt_obj_coord2[:-1],
                    hs[self.begin_l:-1], hs_gt[:-1],  hs_gt2[:-1],
                    att[self.begin_l:-1], att_gt[:-1], att_gt2[:-1]
                    )]
        ans1.extend(ans2)
        return ans1
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_gt_obj_labels2(self, outputs, targets, indices, indices_gt, num_interactions, log=True):
        src_logits = outputs['gt_obj_logits2']
        target_classes_o = torch.cat([t['obj_labels'] for t in targets])
        src_logits = src_logits[indices_gt]
        if len(target_classes_o) == 0:
            loss_obj_ce = src_logits.sum()
            ...
            loss_obj_ce = F.cross_entropy(src_logits, target_classes_o, self.empty_weight)
        losses = {'loss_gt_obj_ce2': loss_obj_ce}
        return losses
    ...
    def loss_gt_verb_labels2(self, outputs, targets, indices, indices_gt, num_interactions):
        src_logits = outputs['gt_verb_logits2']
        target_classes_o = torch.cat([t['verb_labels'] for t in targets])
        src_logits = src_logits[indices_gt]
        ...
        losses = {'loss_gt_verb_ce2': loss_verb_ce}
        return losses
    ...
    def loss_gt_sub_obj_boxes2(self, outputs, targets, indices, indices_gt, num_interactions):
        src_sub_boxes = outputs['gt_sub_boxes2'][indices_gt]
        src_obj_boxes = outputs['gt_obj_boxes2'][indices_gt]
        target_sub_boxes = torch.cat([t['sub_boxes'] for t in targets], dim=0)
        target_obj_boxes = torch.cat([t['obj_boxes'] for t in targets], dim=0)
        ...
        losses = {}
        if src_sub_boxes.shape[0] == 0:
            losses['loss_gt_sub_bbox2'] = src_sub_boxes.sum()
            losses['loss_gt_obj_bbox2'] = src_obj_boxes.sum()
            losses['loss_gt_sub_giou2'] = src_sub_boxes.sum()
            losses['loss_gt_obj_giou2'] = src_obj_boxes.sum()
        else:
            loss_sub_bbox = F.l1_loss(src_sub_boxes, target_sub_boxes, reduction='none')
            loss_obj_bbox = F.l1_loss(src_obj_boxes, target_obj_boxes, reduction='none')
            losses['loss_gt_sub_bbox2'] = loss_sub_bbox.sum() / num_interactions
            losses['loss_gt_obj_bbox2'] = (loss_obj_bbox * exist_obj_boxes.unsqueeze(1)).sum() / (
                    exist_obj_boxes.sum() + 1e-4)
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_sub_boxes),
                                                               box_cxcywh_to_xyxy(target_sub_boxes)))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(src_obj_boxes),
                                                               box_cxcywh_to_xyxy(target_obj_boxes)))
            losses['loss_gt_sub_giou2'] = loss_sub_giou.sum() / num_interactions
            losses['loss_gt_obj_giou2'] = (loss_obj_giou * exist_obj_boxes).sum() / (exist_obj_boxes.sum() + 1e-4)
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, indices_gt, num, **kwargs):
        loss_map = {
            ...
            'gt_obj_labels2': self.loss_gt_obj_labels2,
            'gt_verb_labels2': self.loss_gt_verb_labels2,
            'gt_sub_obj_boxes2': self.loss_gt_sub_obj_boxes2,
...
def build(args):
    ...
    transformer = build_hoi_transformer_ts_qpos_eobj_attention_map_d(args, begin_l=begin_l,
    ...
    weight_dict['loss_gt_obj_ce2'] = args.obj_loss_coef
    weight_dict['loss_gt_verb_ce2'] = args.verb_loss_coef
    weight_dict['loss_gt_sub_bbox2'] = args.bbox_loss_coef
    weight_dict['loss_gt_obj_bbox2'] = args.bbox_loss_coef
    weight_dict['loss_gt_sub_giou2'] = args.giou_loss_coef
    weight_dict['loss_gt_obj_giou2'] = args.giou_loss_coef
    ...
    losses = ['obj_labels', 'verb_labels', 'sub_obj_boxes', 'obj_cardinality',
              'gt_obj_labels', 'gt_verb_labels', 'gt_sub_obj_boxes', 'gt_obj_labels2', 'gt_verb_labels2', 'gt_sub_obj_boxes2']
