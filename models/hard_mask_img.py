...
def multi_head_attention_forward(
    ...
    if key_padding_mask is not None:
        ...
        if use_hard_mask:  # and training:
            rate = 0.4
            key_padding_mask_rand = torch.rand_like(key_padding_mask.float())
            key_padding_mask_rand = key_padding_mask_rand * (~key_padding_mask).float()
            key_padding_mask_rand = key_padding_mask_rand > (1 - rate)
            attn_output_weights = attn_output_weights.masked_fill(
                key_padding_mask_rand.unsqueeze(1).unsqueeze(2),
                float('-inf'),
            )
        #####################
...
class HardImgMaskMultiheadAttention(MultiheadAttention):
    def __init__(self, embed_dim, num_heads, **kwargs):
        super(HardImgMaskMultiheadAttention, self).__init__(embed_dim, num_heads, **kwargs)
