...
from models.DN.doq_components import prepare_for_hqm, doq_post_process, compute_doq_loss
...
class CDNHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, doq_args=None, hard=True, gt_lr_cross=False):
        ...
        if doq_args is not None:
            ...
            input_query_with_gt_position_query, attn_mask, tgt_padding_mask, mask_dict = \
                prepare_for_hqm(doq_args, src.size(0), self.bbox_enc, self.query_embed.weight, hard, gt_lr_cross)
            ####
            hopd_out, interaction_decoder_out = self.transformer(self.input_proj(src), mask,
                                                                 input_query_with_gt_position_query, pos[-1], None,
                                                                 attn_mask)[:2]
