...
class HOIAEvaluator():
    def __init__(self, preds, gts, correct_mat, write_file_name=None):
        self.image_index_to_file_name = json.load(open("/data2/xubin/qpic/data/HOI-A/annotations/image_index_to_file_name.json", "r"))
        ...
        for img_preds in preds:
            ...
            self.preds.append({
                'file_name': self.image_index_to_file_name[str(img_preds["image_id"])],
                'predictions': bboxes,
                'hoi_prediction': hois
            })
        if write_file_name:
