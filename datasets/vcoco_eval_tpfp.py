...
class VCOCOEvaluator():
    ...
    def __init__(self, preds, gts, subject_category_id, correct_mat):
        ...
        self.human_boxes = defaultdict(list)
        self.object_boxes = defaultdict(list)
        self.file_names = defaultdict(list)
        self.obj_cates = defaultdict(list)
        self.image_idx_to_image_name = io.load_json_object(
            "/data2/xubin/qpic/data/v-coco/annotations/image_index_to_file_name.json")
        ...
        for img_gts in gts:
            idx = img_gts["id"]
            file_name = self.image_idx_to_image_name[str(idx)]
    ...
    def evaluate(self):
        for img_preds, img_gts in zip(self.preds, self.gts):
            ...
            if len(gt_bboxes) != 0:
                ...
                self.compute_fptp(pred_hois, gt_hois, bbox_pairs, pred_bboxes, bbox_overlaps, file_name, pred_bboxes)
            else:
                assert False
        ...
        # hoi_list = io.load_json_object(
        #     "/data2/xubin/qpic/data/hico_20160224_det/annotations/hoi_list.json")
        # obj_index_verb_index_hoi_index = {}
        # for hoi in hoi_list:
        #     object_index = hoi["object_index"]
        #     verb_index = hoi["verb_id"] - 1
        #     if object_index not in obj_index_verb_index_hoi_index:
        #         obj_index_verb_index_hoi_index[object_index] = {}
        #     if verb_index not in obj_index_verb_index_hoi_index[object_index]:
        #         obj_index_verb_index_hoi_index[object_index][verb_index] = hoi["id"]
        ...
        for key in self.tp:
            save_resut[str(key)] = {
                "tp": list(self.tp[key]),
                "fp": list(self.fp[key]),
                "file_names": list(self.file_names[key]),
                "score": list(self.score[key]),
                "human_boxes": list(self.human_boxes[key]),
                "object_boxes": list(self.object_boxes[key]),
                "obj_cates": list(self.obj_cates[key]),
            }
        io.dump_json_object(save_resut, os.path.join("/data2/xubin/qpic/outputs/vcoco/test/", "tpfp.json"))
    ...
    def compute_map(self):
        ...
        for category_id in sorted(list(self.sum_gts.keys())):
            ...
            if len(tp) == 0:
                ...
                #############################################
                self.tp[category_id] = np.asarray(self.tp[category_id])[sort_inds]
                self.fp[category_id] = np.asarray(self.fp[category_id])[sort_inds]
                self.human_boxes[category_id] = np.asarray(self.human_boxes[category_id])[sort_inds]
                self.object_boxes[category_id] = np.asarray(self.object_boxes[category_id])[sort_inds]
                self.file_names[category_id] = np.asarray(self.file_names[category_id])[sort_inds]
                self.obj_cates[category_id] = np.asarray(self.obj_cates[category_id])[sort_inds]
                self.score[category_id] = np.asarray(self.score[category_id])[sort_inds]
                #############################################
                fp = fp[sort_inds]
                tp = tp[sort_inds]
                fp = np.cumsum(fp)
    ...
    def compute_fptp(self, pred_hois, gt_hois, match_pairs, pred_bboxes, bbox_overlaps, file_name, pred_bbox):
        ...
        if len(pred_hois) != 0:
            for pred_hoi in pred_hois:
                ...
                self.human_boxes[pred_hoi['category_id']].append(
                    pred_bbox[pred_hoi['subject_id']]["bbox"])
                self.object_boxes[pred_hoi['category_id']].append(
                    pred_bbox[pred_hoi['object_id']]["bbox"])
                self.obj_cates[pred_hoi['category_id']].append(
                    pred_bbox[pred_hoi['object_id']]["category_id"])
                self.file_names[pred_hoi['category_id']].append(file_name)
