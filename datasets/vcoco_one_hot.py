...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries):
        ...
        self.verb_29 = np.load("data/v-coco/annotations/verb_29.npy")
        self.obj_80 = np.load("data/hico_20160224_det/annotations/coco_80.npy")
        self.bg = np.load("data/v-coco/annotations/bg.npy")
        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if hoi['object_id'] == -1:
                    ...
                    obj_vec = self.bg.reshape(1, 300)
                else:
                    obj_labels.append(target['labels'][kept_box_indices.index(hoi['object_id'])])
                    obj_vec = self.obj_80[target['labels'][kept_box_indices.index(hoi['object_id'])]].reshape(1, 300)

                verb_label = [0 for _ in range(len(self._valid_verb_ids))]
                verb_label[self._valid_verb_ids.index(hoi['category_id'])] = 1
                verb_labels_one.append(torch.tensor(self._valid_verb_ids.index(hoi['category_id'])))
                verb_vec = self.verb_29[self._valid_verb_ids.index(hoi['category_id'])].reshape(1, 300)
                ...
                gt_items_ = np.concatenate([sub_box.reshape(1, 4), obj_box.reshape(1, 4),
                                            verb_vec, obj_vec], axis=1)
                gt_items = np.concatenate([gt_items, gt_items_], axis=0)
            ...
            if len(obj_boxes) == 0:
                ...
                target['gt_items'] = torch.zeros((0, 600 + 8), dtype=torch.float32)
            else:
