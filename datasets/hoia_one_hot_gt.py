...
class HOIADetection(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, hard_negative=False):
        ...
        self.hoi_word = np.load("data/HOIA-2021/annotations/hoi.npy")
    ...
    def __getitem__(self, idx):
        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                hoi_label = [0 for _ in range(len(self._valid_hoi_ids))]
                hoi_label[hoi_category_id] = 1
                hoi_labels_vector.append(hoi_label)
                #################################################################################################
                ...
                gt_items_ = np.concatenate(
                    [self.hoi_word[self._valid_hoi_ids.index(hoi_category_id)],
                     sub_box, obj_box, c_dis, wh_size])
                gt_items = np.concatenate([gt_items, gt_items_.reshape(1, 612)], axis=0)

            if len(sub_obj_pairs) == 0:
                target['obj_labels'] = torch.zeros((0,), dtype=torch.int64)
                target['hoi_labels'] = torch.zeros((0,), dtype=torch.int64)
                target['verb_labels'] = torch.zeros((0, len(self._valid_verb_ids)), dtype=torch.float32)
                target['hoi_labels_vector'] = torch.zeros((0, len(self._valid_hoi_ids)), dtype=torch.float32)
                ...
                target['gt_items'] = torch.zeros((0, 612), dtype=torch.float32)
            else:
                target['obj_labels'] = torch.stack(obj_labels)
                target['verb_labels_one'] = torch.stack(verb_labels_one)
                target['hoi_labels'] = torch.stack(hoi_labels)
                target['verb_labels'] = torch.as_tensor(verb_labels, dtype=torch.float32)
                target['hoi_labels_vector'] = torch.as_tensor(hoi_labels_vector, dtype=torch.float32)
