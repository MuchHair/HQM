...
class HICOEvaluator():
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat,output_dir):
        ...
        self.image_idx_to_image_name = io.load_json_object(
            "/data2/xubin/qpic/data/hico_20160224_det/annotations/image_index_to_file_name.json")
    ...
    def evaluate(self):
        ...
        for key in self.tp:
            assert key[0] == 0
            hoi_id = obj_index_verb_index_hoi_index[key[1]][key[2]]
            save_resut[hoi_id] = {
                "tp": self.tp[key],
                "fp": self.fp[key],
                "file_names": self.file_names[key],
                "score": self.score[key],
                "human_boxes": self.human_boxes[key],
                "object_boxes": self.object_boxes[key],
                "obj_cates": self.obj_cates[key],
            }
        io.dump_json_object(save_resut, os.path.join(self.output_dir, "tpfp.json"))
    ...
    def compute_map(self):
        ...
        for triplet in self.gt_triplets:
            ...
            #############################################
            self.tp[triplet] = np.asarray(self.tp[triplet])[sort_inds]
            self.fp[triplet] = np.asarray(self.fp[triplet])[sort_inds]
            self.human_boxes[triplet] = np.asarray(self.human_boxes[triplet])[sort_inds]
            self.object_boxes[triplet] = np.asarray(self.object_boxes[triplet])[sort_inds]
            self.file_names[triplet] = np.asarray(self.file_names[triplet])[sort_inds]
            self.obj_cates[triplet] = np.asarray(self.obj_cates[triplet])[sort_inds]
            self.score[triplet] = np.asarray(self.score[triplet])[sort_inds]
            #############################################
            fp = fp[sort_inds]
            tp = tp[sort_inds]
            fp = np.cumsum(fp)
    ...
    def compute_fptp(self, pred_hois, gt_hois, match_pairs, pred_bboxes, bbox_overlaps, file_name, pred_bbox):
        ...
        if len(pred_hois) != 0:
            for pred_hoi in pred_hois:
                ...
                self.human_boxes[triplet].append(
                    pred_bbox[pred_hoi['subject_id']]["bbox"])
                self.object_boxes[triplet].append(
                    pred_bbox[pred_hoi['object_id']]["bbox"])
                self.obj_cates[triplet].append(
                    pred_bbox[pred_hoi['object_id']]["category_id"])
                self.file_names[triplet].append(file_name)
