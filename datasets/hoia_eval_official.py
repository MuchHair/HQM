...
class hoia():
    ...
    def compute_map(self):
        ...
        for i in list(self.verb_name_dict.keys()):
            ...
            print('class {} --- ap: {}   max recall: {}'.format(i, ap[i - 1], max_recall[i - 1]))
        mAP = np.mean(ap[:])
...
if __name__ == "__main__":
    hoi_eval = hoia(os.path.join("/data2/xubin/qpic/data/HOI-A/annotations/test_2019.json"))
    predict_annot = json.load(
        open("/data2/xubin/qpic/outputs/hoia/baseline_decoder_as_ffn_2l_7l/result_10.json", "r"))
    for pred_i in tqdm(predict_annot):
        pred_bbox = pred_i['predictions']
        pred_hoi = pred_i['hoi_prediction']
        for p in pred_bbox:
            assert 0 <= p["category_id"] < 11
            p["category_id"] = int(p["category_id"]) + 1
        for p in pred_hoi:
            assert 0 <= p["category_id"] < 10
            p["category_id"] = int(p["category_id"]) + 1

    io.dump_json_object(predict_annot, "/data2/xubin/qpic/outputs/hoia/baseline_decoder_as_ffn_2l_7l/result_10_format.json", indent=None)

    print(len(predict_annot))
    map = hoi_eval.evalution(predict_annot)
