    ...
    def __getitem__(self, idx):
        ...
        if self.img_set == 'train':
            ...
            if len(sub_obj_pairs) == 0:
                ...
                target['obj_labels_noni'] = torch.zeros((0,), dtype=torch.int64)
                target['verb_labels_noni'] = torch.zeros((0, len(self._valid_verb_ids)), dtype=torch.float32)
                target['sub_boxes_noni'] = torch.zeros((0, 4), dtype=torch.float32)
                target['obj_boxes_noni'] = torch.zeros((0, 4), dtype=torch.float32)
            else:
                ...
                ###############extra non-interactive pairs
                sub_ids = target['sub_ids']
                obj_ids = target['obj_ids']
                all_ids_cat = torch.cat([sub_ids, obj_ids], dim=0)
                all_ids = all_ids_cat.unique()
                num_boxes = len(all_ids)
                max_id = all_ids.max()
                if max_id >= num_boxes:
                    # sub_ids :tensor([ 0,  3,  5,  7, 10, 12, 14, 16, 19]),
                    # obj_ids :tensor([ 1,  4,  6,  8, 11, 13, 15, 17, 20]),
                    # all_ids: tensor([ 0,  1,  3,  4,  5,  6,  7,  8, 10, 11, 12, 13, 14, 15, 16, 17, 19, 20])
                    sub_ids = (sub_ids[:, None] == all_ids[None, :]).nonzero()[:, 1]
                    obj_ids = (obj_ids[:, None] == all_ids[None, :]).nonzero()[:, 1]
                    all_ids_cat = torch.cat([sub_ids, obj_ids], dim=0)
                    all_ids = torch.arange(num_boxes)
                    # print(f'max_id >= num_boxes: {max_id >= num_boxes}, sub_ids :{sub_ids}, obj_ids :{obj_ids}, all_ids: {all_ids}')

                pair_mat = torch.zeros(num_boxes, num_boxes)
                pair_mat[sub_ids] = (1 - torch.eye(num_boxes))[sub_ids]
                pair_mat[sub_ids, obj_ids] = 0
                pair_ind = pair_mat.nonzero()

                if len(pair_ind):
                    # print(f'sub_ids :{sub_ids}, obj_ids :{obj_ids}, all_ids: {all_ids}\n'
                    #       f'all_ids_cat: {all_ids_cat}, all_ids: {all_ids}\n'
                    #       f'pair_ind: {pair_ind}')

                    all_boxes_cat = torch.cat([target['sub_boxes'], target['obj_boxes']], dim=0)
                    all_boxes_ind = (all_ids[:, None] == all_ids_cat[None, :]).float().argmax(1)
                    all_boxes = all_boxes_cat[all_boxes_ind]
                    all_labels_cat = torch.cat([torch.zeros_like(target['obj_labels']), target['obj_labels']], dim=0)
                    all_labels = all_labels_cat[all_boxes_ind]

                    sub_boxes_noni = all_boxes[pair_ind[:, 0]]
                    obj_boxes_noni = all_boxes[pair_ind[:, 1]]
                    obj_labels_noni = all_labels[pair_ind[:, 1]]

                    # print(f'all_boxes_cat :{all_boxes_cat}\nall_boxes_ind: {all_boxes_ind}\n'
                    #       f'all_boxes: {all_boxes}\nall_labels_cat: {all_labels_cat}\n'
                    #       f'all_labels: {all_labels}\n sub_boxes_noni: {sub_boxes_noni}\n'
                    #       f'obj_boxes_noni: {obj_boxes_noni}\nobj_labels_noni: {obj_labels_noni}')


                    obj_word = torch.from_numpy(self.word2vec_obj)[obj_labels_noni]
                    c_dis = sub_boxes_noni[:, :2] - obj_boxes_noni[:, :2]
                    wh_size = torch.stack([sub_boxes_noni[:, 2] * sub_boxes_noni[:, 3], obj_boxes_noni[:, 2] * obj_boxes_noni[:, 3]], dim=1)

                    gt_items_ = torch.cat([obj_word, sub_boxes_noni, obj_boxes_noni, c_dis, wh_size], dim=1)

                    num_keep = 3 * len(target['sub_boxes'])
                    if len(pair_ind) > num_keep:
                        gt_items_ = gt_items_[:num_keep]
                        obj_labels_noni = obj_labels_noni[:num_keep]
                        sub_boxes_noni = sub_boxes_noni[:num_keep]
                        obj_boxes_noni = obj_boxes_noni[:num_keep]

                    target['gt_items'] = torch.cat([target['gt_items'], gt_items_], dim=0)
                    target['obj_labels_noni'] = obj_labels_noni
                    target['verb_labels_noni'] = torch.zeros((len(obj_labels_noni), len(self._valid_verb_ids)))
                    target['sub_boxes_noni'] = sub_boxes_noni
                    target['obj_boxes_noni'] = obj_boxes_noni
                else:
                    target['obj_labels_noni'] = torch.zeros((0,), dtype=torch.int64)
                    target['verb_labels_noni'] = torch.zeros((0, len(self._valid_verb_ids)), dtype=torch.float32)
                    target['sub_boxes_noni'] = torch.zeros((0, 4), dtype=torch.float32)
