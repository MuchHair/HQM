...
sys.path.insert(0, "/data2/xubin/qpic/")
...
class HICOEvaluator():
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat, output_dir):
        ...
        for img_preds in preds:
            ...
            self.preds.append({
                'file_name': self.image_idx_to_image_name[str(img_preds["image_id"])],
                'predictions': bboxes,
                'hoi_prediction': hois
            })

        if output_dir:
            io.dump_json_object(self.preds, os.path.join(output_dir, "pred.json"))
    ...
    def compute_map(self):
        ...
        for k, v in ap.items():
            assert k[0] == 0
            hoi_id = obj_index_verb_index_hoi_index[k[1]][k[2]]
            write_results[hoi_id] = v

        write_results["mAP"] = m_ap
        write_results["mAP rare"] = m_ap_rare
        write_results["mAP non-rare"] = m_ap_non_rare
        io.dump_json_object(write_results, os.path.join(self.output_dir, "mAP.json"))
