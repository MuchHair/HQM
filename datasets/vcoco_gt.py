...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries):
        ...
        self._valid_hoi_ids = list(range(1, 235))
        self.hoi_word = np.load('data/v-coco/annotations/hoi.npy')
        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs:
                    ...
                    if "hoi_category_id" in hoi:
                        gt_items_ = np.concatenate(
                            [self.hoi_word[self._valid_hoi_ids.index(hoi['hoi_category_id'])][0:300],
                             sub_box, obj_box, c_dis, wh_size])
                    else:
                        gt_items_ = np.concatenate(
                            [self.bg.reshape([300]), sub_box, obj_box, c_dis, wh_size])
