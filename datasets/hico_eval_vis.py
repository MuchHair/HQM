...
class HICOEvaluator():
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat, save_path):
        ...
        dump_json_object(self.preds, f"{self.save_path}/result.json")
        print("save json!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    def evaluate(self):
        for img_preds, img_gts in zip(self.preds, self.gts):
            ...
            if len(gt_bboxes) != 0:
                ...
                self.compute_fptp(pred_hois, gt_hois, bbox_pairs, pred_bboxes, bbox_overlaps, image_id)
            else:
    ...
    def compute_fptp(self, pred_hois, gt_hois, match_pairs, pred_bboxes, bbox_overlaps, image_id):
        ...
        if len(pred_hois) != 0:
            for pred_hoi in pred_hois:
                ...
                if is_match == 1 and vis_tag[gt_hois.index(max_gt_hoi)] == 0:
                    ...
                    w_tp.append(0)
                self.score[triplet].append(pred_hoi['score'])
        np.save(os.path.join(self.save_path, f"{image_id}.npy"), np.array(w_tp))
