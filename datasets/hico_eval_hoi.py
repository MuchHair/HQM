...
class HICOEvaluatorHOI():
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat):
        ...
        self.verb_index_obj_index_hoi_index = load_json_object(
            "data/hico_20160224_det/annotations/verb_index_obj_index_hoi_index.json")

        self.preds = []
        for img_preds in preds:
            img_preds['sub_ids'] = np.array(img_preds['sub_ids'])
            img_preds['obj_ids'] = np.array(img_preds['obj_ids'])
            hoi_scores = np.array(img_preds['verb_scores'])

            hoi_scores1 = np.array(img_preds['hoi_scores'])
            hoi_scores1 = hoi_scores1[:, np.newaxis, :].repeat(hoi_scores.shape[1], axis=1)
            hoi_scores1 = hoi_scores1.reshape(-1, 600)
            ...
            if len(subject_ids) > 0:
                ...
                hois = []
                hois1 = []
                for subject_id, object_id, category_id, score, score1 in zip(subject_ids, object_ids, verb_labels,
                                                                             hoi_scores, hoi_scores1):
                    if score == 0:
                        continue
                    obj_cate = int(bboxes[object_id]["category_id"])
                    assert 0 <= obj_cate < 80
                    assert 0 <= category_id < 117
                    hoi_index = int(self.verb_index_obj_index_hoi_index[str(category_id)][str(obj_cate)])
                    hoi_score = score1[hoi_index]
                    hois.append({'subject_id': subject_id, 'object_id': object_id, 'category_id': category_id,
                                 'score': score * hoi_score})
                    hois1.append({'subject_id': subject_id, 'object_id': object_id, 'category_id': category_id,
                                  'score': score})

                hois.sort(key=lambda k: (k.get('score', 0)), reverse=True)
                hois = hois[:self.max_hois]
                hois1.sort(key=lambda k: (k.get('score', 0)), reverse=True)
                hois1 = hois1[:self.max_hois]
            else:
                hois = []
                hois1 = []
            assert len(hois) == len(hois1)
            self.preds.append({
                'predictions': bboxes,
                'hoi_prediction': hois1,
                'hoi_prediction_hoi': hois
            })
    ...
    def evaluate(self):
        map = {}
        map.update(self.evaluate_verb())
        map.update(self.evaluate_hoi())
        return map

    def evaluate_verb(self):
    ...
    def evaluate_hoi(self):
        ...
        for img_preds, img_gts in zip(self.preds, self.gts):
            pred_bboxes = img_preds['predictions']
            gt_bboxes = img_gts['annotations']
            pred_hois = img_preds['hoi_prediction_hoi']
            gt_hois = img_gts['hoi_annotation']
            if len(gt_bboxes) != 0:
                bbox_pairs, bbox_overlaps = self.compute_iou_mat(gt_bboxes, pred_bboxes)
                self.compute_fptp(pred_hois, gt_hois, bbox_pairs, pred_bboxes, bbox_overlaps)
    ...
    def compute_map_hoi(self):
        ...
        print('mAP: {} mAP rare: {}  mAP non-rare: {}  mean max recall: {}'.format(m_ap, m_ap_rare, m_ap_non_rare,
                                                                                   m_max_recall))
        return {'mAP hoi': m_ap, 'mAP rare hoi': m_ap_rare, 'mAP non-rare hoi': m_ap_non_rare,
                'mean max recall hoi': m_max_recall}
